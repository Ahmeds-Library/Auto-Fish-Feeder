...
- Pairing uses the `claimDevice` Cloud Function, but administrators must still create each device Auth user and set `/devices/{deviceId}/deviceAuthUid` during provisioning.
...
## Pairing portal and cloud claim

The setup portal includes the Device ID, a 6-digit pairing code, and a dashboard pairing link built from `webAppUrl`:

```text
https://your-firebase-hosting-domain.web.app/devices/pair?deviceId=FF-A84F21&code=483921
```

The ESP publishes `/pairingRequests/{pairingCode}` after Firebase connects if the device is not already paired. Requests expire after about 15 minutes and are refreshed only when the prior request is missing, expired, or claimed. QR rendering on ESP8266 is intentionally left as future work; the portal displays the URL as text.
