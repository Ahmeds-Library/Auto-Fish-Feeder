...
String generatePairingCode() {
  randomSeed(ESP.getCycleCount() ^ ESP.getChipId() ^ millis());
  char code[7];
  snprintf(code, sizeof(code), "%06lu", random(0, 1000000));
  return String(code);
}
}
...
void WifiSetupPortal::handleSave() {
  ...
  newConfig.pairingCode = _server.arg("pairingCode").length() ? _server.arg("pairingCode") : generatePairingCode();
  newConfig.webAppUrl = _server.arg("webAppUrl").length() ? _server.arg("webAppUrl") : "https://your-firebase-hosting-domain.web.app";
...
String WifiSetupPortal::htmlPage(const String &message) {
  ...
  String pairingCode = _config->pairingCode.length() ? _config->pairingCode : "Generated after save";
  String webAppUrl = _config->webAppUrl.length() ? _config->webAppUrl : "https://your-firebase-hosting-domain.web.app";
  String pairingLink = webAppUrl + "/devices/pair?deviceId=" + _config->deviceId + "&code=" + pairingCode;
  html += input("adminPin", "Admin PIN * (MVP stores plainly; hash in production)", _config->adminPin, "password");
  html += input("pairingCode", "Pairing Code (6 digits; leave blank to generate)", _config->pairingCode);
  html += input("webAppUrl", "Web App URL", webAppUrl);
  html += "<div class='warn'><strong>Pairing details</strong><br>Device ID: " + (_config->deviceId.length() ? _config->deviceId : "Save config first") + "<br>Pairing code: " + pairingCode + "<br>Pairing link: <br><code>" + pairingLink + "</code><br>After connecting this device to Wi-Fi, open the pairing link in your dashboard account. QR code generation is a future improvement.</div>";
